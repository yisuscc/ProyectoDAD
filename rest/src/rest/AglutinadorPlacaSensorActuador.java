...
public List<Sensor> getLastSensoresList(Integer placaId){
	return getLastSensoresPlaca(placaId).entrySet().stream().map(e-> e.getValue()).toList();
}
	public Sensor getLastSensor(Integer id, Integer placaId) {
		List<Sensor> lsAux = mapaSensores.get(Par.of(id, placaId));
